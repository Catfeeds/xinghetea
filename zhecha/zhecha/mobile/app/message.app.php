...
class MessageApp extends MemberbaseApp
{
    ...
    function newpm()
    {
		...
		$this->_get_curlocal_title('newpm');
    ...
    function privatepm()
    {
        ...
        /* 当前用户中心菜单 */
		$this->_get_curlocal_title('privatepm');
    ...
    function systempm()
    {
		...
		$this->_get_curlocal_title('systempm');
    ...
    function announcepm()
    {
        ...
        /* 当前用户中心菜单 */
        $this->_get_curlocal_title('announcepm');
    ...
    function send()
    {
        ...
        if (!IS_POST){
			...
			$this->assign('to_user_name',$_GET['to_user_name']);

            //引入jquery表单插件
            $this->import_resource(array(
                'script' => 'jquery.plugins/jquery.validate.js',
            ));
			$this->_get_curlocal_title('send_message');
        ...
        else
        {
            ...
            if (!$to_user_name)
            {
                $this->json_error('no_to_user_name'); //没有填写用户名
                return;
            }
            ...
            if (!$to_ids)
            {
                $this->json_error('no_user_self'); //没有该用户名
                return;
            }
            ...
            if (!$msg_id)
            {
                $rs = $ms->pm->get_error();
                $msg = current($rs);
                $this->json_error($msg['msg'], 'go_back', 'index.php?app=message&act=send');
                return;
            }
            $this->json_result('','send_message_successed');
        }
    }
    ...
    function view()
    {
        ...
        if (!IS_POST)
        {
			...
			$this->assign('ret_url',$_SERVER['QUERY_STRING']?'index.php?'.$_SERVER['QUERY_STRING']:'index.php?app=message&act=newpm');
			
			
			$this->_get_curlocal_title('view_message');
        ...
        else
        {
            ...
            {
                $this->json_error('cannot_replay_system_message');
                return;
            }
            ...
            {
                $this->json_error('msg_content_no_empty');
