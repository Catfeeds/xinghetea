...
class DepositApp extends DepositbaseApp
{
	...
	{		
		...
		else
		{
			...
			if(!$deposit_account && (!is_email($account) && !is_mobile($account))) {
	...
	function tradelist()
	{
        ...
        $this->import_resource(array(
            'script' => array(
                array(
                    'path' => 'mobile/date_picker/js/date.js'
                ),
                array(
                    'path' => 'mobile/date_picker/js/iscroll.js'
                )
            ),
            'style' =>  'mobile/date_picker/css/common.css',
        ));
