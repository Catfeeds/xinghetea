...
class My_favoriteApp extends MemberbaseApp
{
    ...
    function add()
    {
        ...
        $store_id = empty($_GET['store_id'])  ? 2 : intval($_GET['store_id']);//增加店铺id参数，兼容产品共用 by PwordC
        if (!$item_id)
        {
        ...
        if ($type == 'goods')
        {
            $this->_add_collect_goods($item_id, $keyword,$store_id);
        }
        elseif ($type == 'store')
        {
    ...
    function _list_collect_goods()
    {
        ...
        $collect_goods = $model_goods->find(array(
            ...
            'fields'=> 'this.*,collect.store_id,collect.add_time,goodsspec.price,goodsspec.spec_id',
		...
		$this->_get_curlocal_title('collect_goods');
    ...
    function _list_collect_store()
    {
		...
		$this->_get_curlocal_title('collect_store');
     ...
     *    @return    void
     */
    function _add_collect_goods($goods_id, $keyword,$store_id)
    {
        /* 验证要收藏的商品是否存在 */
        $model_goods =& m('goods');
        $goods_info  = $model_goods->get($goods_id);
        
        //赋值产品store_id，兼容共用产品 by PwordC
        ...
        //重写添加收藏，为了兼容共用产品， by PwordC
        $db =& db();
        $now = time();
        //插入sql
        $insert_sql = "INSERT INTO ecm_collect(user_id,type,item_id,keyword,add_time,store_id) values({$this->visitor->get('user_id')},'goods',{$goods_id},'{$keyword}',{$now},{$store_id})";
        //先检查是否已收藏
        $check_sql = "select * from ecm_collect where item_id={$goods_id} and store_id={$store_id}";
        $count = $db->getRow($check_sql);
        if (!$count){
            $db->query($insert_sql);
        }
//         $model_user =& m('member');
...
//         ));
        ...
        $goods_url  = SITE_URL . '/' . url('app=goods&id=' . $goods_id.'&store_id='.$store_id);//增加store_id，兼容产品共用 by PwordC
