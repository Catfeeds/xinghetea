...
class GroupbuyApp extends StorebaseApp
{
   ...
   function index()
   {
        ...
        if ($goods['closed'] == 1)
        {
            $this->json_error('groupbuy_goods_closed');
            return;
        }
        ...
        if (!IS_POST)
        {
			...
			$group['end_time'] > gmtime() && $group['left_time']=Psmb_init()->lefttime($group['end_time']);
            // 团购规格价格
            $group['spec_price'] = unserialize($group['spec_price']);
			
			//by psmoban.com
			$group_price=current($group['spec_price']);
			$group['group_price']=$group_price['price'];
            // 可执行操作
            ...
            /* 页面标题 */
			$this->_get_curlocal_title($group['group_name']);
        ...
        else
        {
                if (isset($_POST['join']))
                {
                    ...
                    foreach ($_POST['quantity'] as $key => $val)
                    {
                        ...
                        {
                            $this->json_error('invalid_quantity');
                            return;
                        }
                    }
                    if ($quantity == 0)
                    {
                        $this->json_error('fill_quantity');
                        return;
                    }
                    if ($group['max_per_user'] > 0 && $quantity > $group['max_per_user'])
                    {
                        $this->json_error(sprintf(Lang::get('error_max_per_user'), $group['max_per_user']));
                        return;
                    }
                    ...
                    if (!$link_man || !$tel)
                    {
                        $this->json_error('fill_join_user_info');
                        return;
                    }
					...
					$this->json_result('','join_groupbuy_successed');
                    return;
                }
                elseif (isset($_POST['qa']))
                {
                    ...
                    if ($qa_mod->add($data))
                    {
						$this->json_result('','question_successful');
                        return;
                    }
                    else
                    {
                        $this->json_error('post_fail');
                        exit;
                    }
                ...
                else if (isset($_POST['exit']))
                {
                    ...
                    $this->json_result('','exit_groupbuy_successed');
                    return;
                }
    ...
    function _query_goods_info($goods_id)
    {
		...
		$g_evaluation=$ordergoods_mod->get(array(
			'conditions'=>'goods_id='.$goods_id.' AND evaluation_status=1 AND is_valid=1',
			'join'      => 'belongs_to_order',
			'fields'        => 'COUNT(*) as evaluation_count,SUM(goods_evaluation) as total',
		));
		$goods['avg_g_eva']=$g_evaluation['evaluation_count'] > 0 ? $g_evaluation['total']/$g_evaluation['evaluation_count'] : 0;
        return $goods;
    }
    ...
    function _get_groupbuy_qa($id)
    {
		...
		if(!empty($qa_info))
		{
			foreach($qa_info as $key=>$qa)
			{
				$qa_info[$key]['portrait'] = portrait($qa['user_id'],$qa['portrait'], 'middle');
			}
		}
