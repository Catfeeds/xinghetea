...
class SearchApp extends MallbaseApp
{
    ...
    function index()
    {
		...
		$this->assign('price_count', count($stats['by_price']));
        ...
        $this->assign('selected_cate',$this-> _get_selected_cate($cate_id));
        
        /* 配置seo信息 */
        $this->_config_seo($this->_get_seo_info('goods', $cate_id));
		$this->_get_curlocal_title(Lang::get('searched_goods'));
        $this->display('search.goods.html');
    }
	
	function _get_selected_cate($cate_id)
    {
        if ($cate_id)
        {
            $gcategory_mod =& bm('gcategory');
            $selected_cate = $gcategory_mod->get($cate_id);
        }
  ...
  /* 搜索店铺 */
    function store()
    {
		...
		if($scategorys)
		{
			foreach($scategorys as $k=>$v)
			{
				$scategorys[$k]['count'] = count($v['children']);
			}
		}
		...
		$this->_get_curlocal_title(Lang::get('searched_stores'));
	...
	function form()
	{
		$decoration_mod = &af('decoration',array('template'=>$this->_get_template_name(),'type'=>'mobile'));
		$data = $decoration_mod->_get_decoration_data('keyword');
		$this->assign('keyword',$data['keyword']);
		
		$this->_config_seo(array(
            'title' => Conf::get('site_title') . ' - ' . Lang::get('searched_goods_box'),
        ));
		$this->_get_curlocal_title('searched_goods_box');
		$this->display('search.form.html');
	}
	...
	function groupbuy()
    {
        ...
        $this->assign('orders', $orders);
        // 当前位置
		$this->_get_curlocal_title('groupbuy');
