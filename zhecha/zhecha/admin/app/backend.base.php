...
class BackendApp extends ECBaseApp
{
    ...
    function login()
    {
        ...
        else
        {
            ...
            $this->json_result(array('ret_url'=>'index.php'), 'login_successed');
        }
    }
    ...
    function _do_login($user_id)
    {
        ...
        $user_info = $mod_user->get(array(
            'conditions' => $user_id,
            'join'       => 'manage_mall',
            'fields'     => 'this.user_id, user_name, reg_time, last_login, last_ip, privs, locked'
        ));
		if(($user_info['privs'] != 'all') && isset($user_info['locked']) && $user_info['locked'])
		{
