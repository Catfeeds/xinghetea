...
class GcategoryApp extends BackendApp
{
    ...
    function index()
    {
        ...
        $this->import_resource(array(
            'script' => 'inline_edit.js,ajax_tree.js',
    ...
    function add()
    {
        ...
        else
        {
            ...
            {
                $this->json_error('max_layer_error');
                return;
            }
            ...
            if (!$cate_id)
            {
				$error = current($this->_gcategory_mod->get_error());
                $this->json_error($error['msg']);
                return;
            }

            $this->json_result(array('ret_ulr'=>'index.php?app=gcategory'),'add_ok');
        }
    }
    ...
    function edit()
    {
        ...
        else
        {
            ...
            $this->json_result(array('ret_ulr'=>'index.php?app=gcategory'),'edit_ok');
        }
    }
    ...
    function ajax_col()
    {
	   ...
	   else if($column == 'link_url')
	   {
		   ...
		   $uploadedfile_mod->edit($id, $data);
           if(!$uploadedfile_mod->has_error())
           {
               echo ecm_json_encode(true);
           }
	   }
    ...
    function batch_edit()
    {
        ...
        else
        {
            ...
            $this->json_result(array('ret_ulr'=>'index.php?app=gcategory'),'batch_edit_ok');
        }
    }
    ...
    function drop()
    {
        ...
        if (!$id)
        {
            $this->json_error('no_gcategory_to_drop');
            return;
        }
    ...
    function import()
    {
        ...
        else
        {
            ...
            {
                $this->json_error('select_file');
                return;
            }
            if ($file['name'] == basename($file['name'],'.csv'))
            {
                $this->json_error('not_csv_file');
                return;
            }
            ...
            $this->json_result(array('ret_ulr'=>'index.php?app=gcategory'),'import_ok');
        }
    }
	...
	function gads()
	{
		$id = intval($_GET['id']);
		$gcategories = $this->_gcategory_mod->get_list(0);
        if (!$id || !in_array($id,array_keys($gcategories)))
        {
            $this->json_error('Hacking Attempt');
            return;
        }
		$gcategory = $this->_gcategory_mod->get($id);
		$this->assign("gcategory", $gcategory);
		$uploadedfile_mod = &m('uploadedfile');
		$files_belong_gcategory = $uploadedfile_mod->find(array(
                'conditions' => 'store_id = 0 AND belong = ' . BELONG_GCATEGORY . ' AND item_id=' . $id,
                'fields' => 'this.file_id, this.file_name, this.file_path,this.link_url',
                'order' => 'add_time DESC'
        ));
		$this->assign('files_belong_gcategory', $files_belong_gcategory);
		$this->assign("id", $id);
        $this->assign("belong", BELONG_GCATEGORY);
		$this->import_resource(array('script' => 'change_upload.js,inline_edit.js'));
		$this->display('gcategory.gads.html');
	}
