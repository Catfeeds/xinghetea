...
class DbApp extends BackendApp
{
    ...
    function drop()
    {
        ...
        if (!$backup_names)
        {
            $this->json_error('no_backup_name');
            return;
        }
    ...
    function import()
    {
        ...
        if ($total != count($backups) || !$total){//检查卷数量
    ...
    function _dump_queue($vol, $vol_size)
    {
        ...
        $this->dump_sql .= "-- END ECMall 2.0 SQL Dump Program ";
        if (empty($tables))
        {
