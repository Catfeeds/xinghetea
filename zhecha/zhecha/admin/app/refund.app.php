	...
	function get_xml()
	{
		...
		if ($_POST['query'] != '') 
		{
			if($_POST['qtype'] == 'buyer_name')
			{
				...
				$conditions .= " AND buyer_id ".db_create_in($user_ids);
			}
			elseif($_POST['qtype'] == 'store_name')
			{
				$stores = $this->_store_mod->find(array(
					'conditions'=>"store_name like '%".$_POST['query']."%'",
					'fields' => 'store_id',
				));
				...
				$conditions .= " AND seller_id ".db_create_in($stores_ids);
			}
			else
			{
