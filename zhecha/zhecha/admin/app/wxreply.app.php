...
class WxreplyApp extends BackendApp
{
        ...
        else
        {
			...
			if($type)
			{
				...
				if(!empty($_FILES['image']))
				{
					...
					$image && $data['image'] = $image['file_path'];
				}
			}
			...
			$image['file_id'] && $upload_mod->edit($image['file_id'],array('item_id' => $reply_id));
	...
	function edit()
	{
        ...
        else
        {
			...
			$upload_mod =& m('uploadedfile');
			$image['file_id'] && $upload_mod->edit($image['file_id'],array('item_id' => $id));
    ...
    function drop()
    {
		...
		//删除图文回复图片
		$upload_mod =& m('uploadedfile');
		$files = $upload_mod->find('belong='.BELONG_WEIXIN.' AND item_id '. db_create_in($ids));
		if(!empty($files)){
			$file_ids = array_keys($files);
			foreach($file_ids as $file_id){
				$upload_mod->drop($file_id);
			}
		}
	...
	function _upload_image()
    {
        ...
        if ($file_path = $uploader->save('data/files/mall/weixin', $uploader->random_filename()))
        {
			...
			$data = array(
				...
				'file_path' => $file_path,
				'add_time'  => gmtime(),
				'belong'	=> BELONG_WEIXIN,
			);
			$upload_mod =& m('uploadedfile');
			$file_id = $upload_mod->add($data);
			if (!$file_id)
			{
				$error = current($upload_mod->get_error());
            	$this->json_error($error['msg']);
            	return false;
			}
            return array('file_id' => $file_id,'file_path' => $file_path);
        }
        else
        {
            return false;
