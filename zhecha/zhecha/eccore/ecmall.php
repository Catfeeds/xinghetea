...
/* 字符串部分用星号代替（缺陷：有中文情况下星号数量尚不能自动对应）*/
function cut_str($string, $startLen = 1, $endLen = 1)
{
	if (function_exists('mb_substr')) {
		if(!empty($string)) {
			$string = mb_substr($string, 0, $startLen, CHARSET) . 
			getXingString($string, $startLen + $endLen) . mb_substr($string, -$endLen, $endLen, CHARSET);
		}
	}
	return $string;
}
/* 尚未处理中文字符长度的情况 */
function getXingString($string, $showLen = 1) {
	$xing = '';
...
function isYunDabao()
{	
	if ( strpos($_SERVER['HTTP_USER_AGENT'], 'CK 2.0') !== false ) {
		return true;
	}	
	return false;
}
...
function logResult1($key = '', $word = '') {
	
	if(is_array($word)) $word = json_encode($word);
		
	$path = ROOT_PATH . "/logs/" . date('Ymd', time());
	@mkdir($path, 0777, true);
		
	$fp = fopen($path ."/log.txt","a");
	flock($fp, LOCK_EX) ;
	fwrite($fp,$key."：执行日期：".date("Y-m-d H:i:s",time())."\n".$word." -IP:" . real_ip() . "\n");
	flock($fp, LOCK_UN);
	fclose($fp);
}
/**
 ...
 * by PwordC
 */
function addLog($name,$data,$ps = ''){
    if (is_array($data)) {
        $data = json_encode($data);
    }
    $date = date('Ymd',time());
    $txt_name = ROOT_PATH."/temp/logs/".$name.$date.".log";
    $content_add_time = date('Y-m-d H:i:s',time());
    $content = $content_add_time." >>>> ".$data."===PS===".$ps."\r\n";
    file_put_contents($txt_name, $content,FILE_APPEND|LOCK_EX);
    return null;
}
 ...
 *
 *    @author    PwordC
 *    @param     string     $tablename  表名，如ecm_batch_log,只传'batch_log'即可;
 *    @param     string     $action     操作名称
 *    @param     string     $status     操作状态,true or false;
 *    @param     string|array    $data       日志内容，可传数组或字符串。数组会转成json存储。
 */
function addLog_to_db($tablename,$action,$status,$data,$user_id,$user_name){
    if (is_array($data)) {
        $data = json_encode($data);
    }
    $table_mod =& m($tablename);   
    $tablename = 'ecm_'.$tablename;   
    $insert_data=array();
