...
<div id="rightTop">
  ...
  <ul class="subnav">
    ...
    <li><span>触屏版主题</span></li>
    <?php else: ?>
    <li><span>电脑版主题</span></li>
...
<ul id="rightCon">
	<?php if ($_GET['type'] == 'mobile'): ?>
	<h3> 当前您使用的主题是 : <em><?php echo $this->_var['curr_wap_template_name']; ?>&nbsp;&nbsp;<?php echo $this->_var['curr_wap_style_name']; ?></em> </h3>
    <?php else: ?>
...
?>
    <li>
        ...
        <div class="templet_style"><img src="<?php echo $this->_var['site_url']; ?><?php if ($_GET['type'] == 'mobile'): ?>/mobile<?php endif; ?>/themes/mall/<?php echo $this->_var['template_name']; ?>/styles/<?php echo $this->_var['style_name']; ?>/preview.jpg" onclick="preview_theme('<?php echo $this->_var['template_name']; ?>', '<?php echo $this->_var['style_name']; ?>','<?php echo $_GET['type']; ?>');" /></div>
        ...
        <input type="submit" value="使用" onclick="goConfirm('您选择的该主题模板与当前使用的主题模板不一致，因此您当前的挂件设置将不能在新主题中显示，您需要重新设置，您确定要使用该模板吗？', 'index.php?app=theme&act=set&template_name=<?php echo $this->_var['template_name']; ?>&style_name=<?php echo $this->_var['style_name']; ?>',true);" class="formbtn" />&nbsp;&nbsp;
        ...
        <?php endif; ?> 
        <?php else: ?>
        <input type="button" value="预览" onclick="preview_theme('<?php echo $this->_var['template_name']; ?>', '<?php echo $this->_var['style_name']; ?>','<?php echo $_GET['type']; ?>');" class="formbtn" />&nbsp;&nbsp;
        <?php if ($this->_var['curr_wap_template_name'] != $this->_var['template_name'] || $this->_var['curr_wap_style_name'] != $this->_var['style_name']): ?>
        ...
        <?php else: ?>
		<input type="submit" value="编辑" onclick="go('<?php echo url('app=theme&act=config&template=' . $this->_var['template_name']. '&style=' . $this->_var['style_name']. '&type=' . $_GET['type']. ''); ?>');" class="formbtn" />
        <?php endif; ?>
        <?php endif; ?>
        </div>
    </li>
