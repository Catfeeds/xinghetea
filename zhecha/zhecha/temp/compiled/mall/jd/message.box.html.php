			...
			<div class="public table">
				<table class="table_head_line" style="margin-top:30px;">
					...
					<tr class="gray line_bold" > 
						...
						<th class="align1" colspan="4"> <label for="all"><span class="all">全选</span></label>
							<a href="javascript:;" class="delete" uri="index.php?app=message&act=drop" name="msg_id" presubmit="confirm('您确定要删除它吗？')" ectype="batchbutton">删除</a> </th>
						<?php else: ?>
						<th class="width1"></th>
						<th colspan="5"></th>
						<?php endif; ?> 
...
?>
						...
						<tr 
					<?php if (($this->_foreach['v']['iteration'] == $this->_foreach['v']['total'])): ?>class="line_bold"<?php else: ?>class="line"<?php endif; ?>>
						...
						<td class="width13"><img class="makesmall" max_width="48" max_height="48" src="<?php echo $this->_var['message']['user_info']['portrait']; ?>" /><span class="user_name"><?php echo $this->_var['message']['user_info']['user_name']; ?></span></td>
							<td 
						<?php if ($this->_var['message']['new'] == 1): ?>class="link2 font_bold"<?php else: ?>class="link2"<?php endif; ?>><?php echo sub_str($this->_var['message']['content'],110); ?>
							</td>
					...
					<?php else: ?>
					<tr class="operations">
						<th colspan="5"> <p class="position2"> <?php echo $this->fetch('member.page.bottom.html'); ?> </p>
						</th>
					</tr>
					<?php endif; ?> 
					<?php endif; ?>
