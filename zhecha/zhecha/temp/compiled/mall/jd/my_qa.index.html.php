			...
			<div class="public_index table1">
				<table>
					...
					<tr class="line_bold">
						...
						<th class="align1" colspan="3"> <label for="all"><span class="all">全选</span></label>
							<a href="javascript:void(0);" class="delete" ectype="batchbutton" uri="index.php?app=my_qa&amp;act=drop" name="id" presubmit="confirm('您确定要删除这些咨询吗')">删除</a> </th>
					</tr>
					<?php endif; ?> 
...
?>
					<tr class="bgcolor">
						...
						<td ><a href="<?php if ($this->_var['my_qa_list']['type'] == 'goods'): ?><?php echo url('app=' . $this->_var['my_qa_list']['type']. '&id=' . $this->_var['my_qa_list']['item_id']. '&act=qa'); ?><?php else: ?><?php echo url('app=' . $this->_var['my_qa_list']['type']. '&id=' . $this->_var['my_qa_list']['item_id']. ''); ?><?php endif; ?>" target="_blank" class="link3"><?php echo $this->_var['my_qa_list']['item_name']; ?></a> &nbsp;&nbsp;[<?php echo $this->_var['lang'][$this->_var['my_qa_list']['type']]; ?>] </td>
						<td width="150"><span class="table_user"><?php if ($this->_var['my_qa_list']['user_name'] != ''): ?><?php echo htmlspecialchars($this->_var['my_qa_list']['user_name']); ?><?php else: ?>游客<?php endif; ?></span></td>
						<td style="width:130px"><a href="javascript:drop_confirm('您确定要删除这些咨询吗', 'index.php?app=my_qa&amp;act=drop&id=<?php echo $this->_var['my_qa_list']['ques_id']; ?>');" class="delete">删除</a>&nbsp;&nbsp; 
					...
					<tr>
						<td></td>
						<td><h3>咨询内容:</h3>
							<p class="float-left pt10 pb10 pl10"><?php echo htmlspecialchars($this->_var['my_qa_list']['question_content']); ?></p></td>
					...
					<?php if ($this->_var['my_qa_list']['reply_content'] != ''): ?>
						...
						<tr 
					<?php if (($this->_foreach['qa']['iteration'] == $this->_foreach['qa']['total'])): ?>class="line_bold"<?php endif; ?>>
					
						<td></td>
						<td><h3><span class="color8">我的回复:</span></h3>
							<p class="float-left pt10 pb10 pl10"><span class="color3"><?php echo htmlspecialchars($this->_var['my_qa_list']['reply_content']); ?></span></p></td>
					...
					<tr>
						...
						<th colspan="3"> <p class="position1">
								<label for="all2"><span class="all">全选</span></label>
								<a href="javascript:void(0);" class="delete" ectype="batchbutton" uri="index.php?app=my_qa&amp;act=drop" name="id" presubmit="confirm('您确定要删除这些咨询吗')">删除</a> </p>
