		...
		<div class="wrap">
			<div class="public">
				<form method="post" enctype="multipart/form-data" id="goods_form">
					<h2 class="title"> 第一步：导入CSV文件<?php if ($this->_var['step'] == 2): ?>
						<label class="error right">OK!<?php endif; ?></label>
						<a title="close" class="fold <?php if ($this->_var['step'] == 2): ?>span_open<?php else: ?>span_close<?php endif; ?>"></a> </h2>
					...
					<table class="table_text word_spacing5" style="margin-top:15px">
						<tr>
							...
							<td><input type="file" name="csv" id="csv" />
								<span class="gray">导入程序默认从第二行执行导入，请保留CSV文件第一行的标题行，最大2M</span></td>
						</tr>
						<tr>
							<td valign="top">商品分类:
								</th>
                    ...
                    <?php echo $this->html_options(array('options'=>$this->_var['mgcategories'])); ?>
									...
									<span class="gray">请选择商品分类（必须选到最后一级）</span> </div></td>
						</tr>
						<tr>
							<td>本店分类:
								</th>
							<td><div class="f_l select">
                  ...
                  <?php echo $this->html_options(array('options'=>$this->_var['sgcategories'])); ?>
								...
								<a class="btn" id="add_sgcategory" href="javascript:;">新增</a> <span class="gray">可以从属于多个本店分类</span></td>
						</tr>
						<tr>
							<td valign="top">字符编码:
								</th>
					...
					<h2 class="title"> 第二步：上传商品图片
						<label style="display:none" class="error right">OK!</label>
						<a title="close" class="fold <?php if ($this->_var['step'] == 2): ?>span_close<?php else: ?>span_open<?php endif; ?>"></a> </h2>
					...
					<table class="word_spacing5">
						...
						<tr id="trUploadContainer">
							<td height="40" valign="top"><div id="divSwfuploadContainer" style="width:740px;">
									<div id="divButtonContainer"> <span id="spanButtonPlaceholder"></span> </div>
									<div id="divFileProgressContainer"></div>
								</div>
								<div id="warning"></div></td>
						</tr>
					</table>
					</ul>
					</td>
