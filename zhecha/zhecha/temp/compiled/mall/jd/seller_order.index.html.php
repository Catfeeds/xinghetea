	...
	<div id="right"> <?php echo $this->fetch('member.curlocal.html'); ?>
		...
		<div class="wrap">
			<div class="public_index table"> 
				...
				<table>
					<tr class="line_bold">
						<th colspan="8"> <div class="search_order clearfix">
					...
					<tr class="operations">
						<th colspan="8"> <p class="position1 clearfix">
								...
								<a href="javascript:;" class="delete" ectype="batchcancel" uri="index.php?app=seller_order&act=cancel_order" name="order_id">取消订单</a> </p>
...
?>
					...
					<tr class="line-hd">
						<th colspan="8" class="clearfix"> <p class="float-left"> <input type="checkbox" value="<?php echo $this->_var['order']['order_id']; ?>" class="checkitem J_CheckItem" <?php if ($this->_var['order']['status'] != ORDER_PENDING && $this->_var['order']['status'] != ORDER_SUBMITTED): ?> disabled="disabled" <?php endif; ?>/>
								<label>订单号：</label>
...
?>
					<tr class="line<?php if (! $this->_var['order']['order_gift'] && ($this->_foreach['fe_goods']['iteration'] == $this->_foreach['fe_goods']['total'])): ?> last_line<?php endif; ?>">
						<td valign="top" class="first clearfix"><div class="pic-info float-left"> <a href="<?php echo url('app=goods&id=' . $this->_var['goods']['goods_id']. '&store_id=' . $this->_var['order']['seller_id']. ''); ?>" target="_blank"><img src="<?php echo $this->_var['goods']['goods_image']; ?>" width="50" height="50" /></a> </div>
...
?>
					<tr class="line <?php if (($this->_foreach['fe_goods']['iteration'] == $this->_foreach['fe_goods']['total'])): ?> last_line<?php endif; ?>">
                        <td valign="top" class="first clearfix relative">
                        	<div class="pic-info float-left">
                            	<a href="<?php echo url('app=gift&id=' . $this->_var['goods']['goods_id']. ''); ?>" target="_blank"><img src="<?php echo $this->_var['goods']['default_image']; ?>" width="50" height="50" /></a>
                            </div>
                            <div class="txt-info float-left">
                            	<div class="txt">
                                	<a href="<?php echo url('app=gift&id=' . $this->_var['goods']['goods_id']. ''); ?>" target="_blank"><?php echo $this->_var['goods']['goods_name']; ?></a>
                                </div>
					...
					<?php else: ?>
					<tr class="sep-row"><td colspan="8"><div class="notice-word"><p>没有符合条件的订单</p></div></td></tr>
					<?php endif; ?>
