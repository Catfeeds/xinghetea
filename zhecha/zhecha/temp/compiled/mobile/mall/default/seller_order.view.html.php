...
?>
			<div class="list clearfix">
				<div class="pic float-left"><img src="<?php echo $this->_var['goods']['goods_image']; ?>" width="50" height="50"  /></div>
				<div class="text float-left"> <?php if (! $this->_var['group_id']): ?><a href="<?php echo url('app=goods&id=' . $this->_var['goods']['goods_id']. '&store_id=' . $this->_var['order']['seller_id']. ''); ?>">
					<h3>
					<?php else: ?> <a target="_blank" href="<?php echo url('app=groupbuy&id=' . $this->_var['group_id']. ''); ?>">
					<h3><strong>[团购] </strong><?php endif; ?><?php echo htmlspecialchars($this->_var['goods']['goods_name']); ?></h3>
					<span><?php echo htmlspecialchars($this->_var['goods']['specification']); ?></span> </a> </div>
				<div class="pri float-right"> <?php echo price_format($this->_var['goods']['price']); ?> x <?php echo $this->_var['goods']['quantity']; ?> </div>
			</div>
...
?>
			<div class="list clearfix">
				<div class="pic float-left"><img src="<?php echo $this->_var['goods']['default_image']; ?>" width="50" height="50"  /></div>
				<div class="text float-left"> <a href="<?php echo url('app=gift&id=' . $this->_var['goods']['goods_id']. ''); ?>">
					<h3><?php echo htmlspecialchars($this->_var['goods']['goods_name']); ?></h3></a> <em class="label-gift">赠品</em></div>
				<div class="pri float-right"> <?php echo price_format($this->_var['goods']['price']); ?> x <?php echo $this->_var['goods']['quantity']; ?> </div>
			</div>
			<?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
			...
			<div class="o-g-bt">
				...
				<a href="<?php echo url('app=seller_order'); ?>" class="btn">返回订单列表</a> </div>
		</div>
		<div class="shipping-info inclusive-box f66">
			<dl>
				...
				<dd>
					<label>上架 :</label>
					<span> <?php echo (local_date("Y-m-d H:i:s",$this->_var['order']['add_time']) == '') ? '-' : local_date("Y-m-d H:i:s",$this->_var['order']['add_time']); ?></span></dd>
		...
		<div class="shipping-info inclusive-box f66">
			<dl>
				...
				<dd>
					<label>买家 :</label>
					<span> <?php echo htmlspecialchars($this->_var['order']['buyer_name']); ?></span></dd>
				<dd>
					<label>电话号码 :</label>
					<!--<span> <?php echo ($this->_var['order']['phone_tel'] == '') ? '-' : $this->_var['order']['phone_tel']; ?></span>--><a href="tel:<?php echo $this->_var['order']['phone_tel']; ?>" style="padding-left:20px;background:url(<?php echo $this->res_base . "/" . 'images/botel.png'; ?>) no-repeat;">拨打电话</a></dd>
				<dd>
					<label>手&nbsp;&nbsp;&nbsp;机 :</label>
					<!--<span> <?php echo ($this->_var['order']['phone_mob'] == '') ? '-' : $this->_var['order']['phone_mob']; ?></span>--><a href="tel:<?php echo $this->_var['order']['phone_mob']; ?>" style="padding-left:20px;background:url(<?php echo $this->res_base . "/" . 'images/bophone.png'; ?>) no-repeat;">拨打手机</a></dd>
				<dd>
					<label>所在地区 :</label>
					<span><?php echo (htmlspecialchars($this->_var['order']['region_name']) == '') ? '-' : htmlspecialchars($this->_var['order']['region_name']); ?></span></dd>
				<dd>
					<label>电子邮件 :</label>
					<span><?php echo ($this->_var['order']['buyer_email'] == '') ? '-' : $this->_var['order']['buyer_email']; ?></span></dd>
				<dd>
					<label>详细地址 :</label>
					<span><?php echo (htmlspecialchars($this->_var['order']['address']) == '') ? '-' : htmlspecialchars($this->_var['order']['address']); ?></span></dd>
			</dl>
		</div>
		<div  class="shipping-info inclusive-box f66" style="margin-bottom:15px;">
			<dl>
				<dt>收货人及物流信息</dt>
				<dd>
					<label>收货地址 :</label>
