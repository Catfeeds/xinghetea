...
class Promotool extends Object
{
	...
	function getItemProInfo($goods_id, $spec_id = 0)
	{
		...
		if(!$this->_store_id)
		{
		    //暂定写死store_id=2,by PwordC
// 			$goods_mod = &m('goods');
// 			$goods = $goods_mod->get(array('conditions' => 'goods_id='.$goods_id, 'fields' => 'store_id'));
			$this->_store_id = 2;
		}
		...
		if($result === FALSE) {
			...
			$proPrice = $limitbuy_mod->get_limitbuy_price($goods_id, $spec_id);
		 	($proPrice !== FALSE) && $result = array('pro_price' => $proPrice, 'pro_type' => 'limitbuy');
		}
		...
		if($result === FALSE) {
			if(Psmb_init()->check_view_device('', FALSE)) {
				...
				$proPrice = $exclusive_mod->get_limitbuy_price($goods_id, $spec_id);
