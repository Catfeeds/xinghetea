...
function spec_update(){
    ...
    SPEC.specs && $.each(SPEC.specs,function(i,item){
        ...
        //添加会员多等级价格 by PwordC
        if ($('#price_1').length > 0){
        	tpl.find('*[item="price_1"]').append('<input type="hidden" name="price_1['+ item.spec_id +']" value="' + item.price_1 + '" />' + item.price_1);
        }
        if ($('#price_2').length > 0){
        	tpl.find('*[item="price_2"]').append('<input type="hidden" name="price_2['+ item.spec_id +']" value="' + item.price_2 + '" />' + item.price_2);
        }
        if ($('#price_1').length > 0){
        	tpl.find('*[item="price_3"]').append('<input type="hidden" name="price_3['+ item.spec_id +']" value="' + item.price_3 + '" />' + item.price_3);
        }
        if ($('#price_1').length > 0){
        	tpl.find('*[item="price_4"]').append('<input type="hidden" name="price_4['+ item.spec_id +']" value="' + item.price_4 + '" />' + item.price_4);
        }
        if ($('#price_1').length > 0){
        	tpl.find('*[item="price_5"]').append('<input type="hidden" name="price_5['+ item.spec_id +']" value="' + item.price_5 + '" />' + item.price_5);
        }
...
function spec_editor(){
    ...
    //兼容会员多价格，计算宽度  by PwordC
    var width = (($('li.distance1').length/2) - 2)*80;
    //console.log(width); 
    _d.setWidth(550+width);
    ...
    $('*[ectype="save_spec"]').click(function(){
        ...
        $('#dialog_object_spec_editor').find('*[ectype="data"]').each(function(){
            ...
            //获取会员等级价格 by PwordC
            if ($('#price_1').length > 0){
            	var price_1 = $.trim($(this).find('*[item="price_1"]').val());
            }
            if ($('#price_2').length > 0){
            	var price_2 = $.trim($(this).find('*[item="price_2"]').val());
            }
            if ($('#price_3').length > 0){
            	var price_3 = $.trim($(this).find('*[item="price_3"]').val());
            }
            if ($('#price_4').length > 0){
            	var price_4 = $.trim($(this).find('*[item="price_4"]').val());
            }
            if ($('#price_5').length > 0){
            	var price_5 = $.trim($(this).find('*[item="price_5"]').val());
            }
            ...
            item != ';' && SPEC.specs.push({
                ...
                'price_1':number_format(price_1, 2),//添加会员多等级价格 by PwordC
                'price_2':number_format(price_2, 2),
                'price_3':number_format(price_3, 2),
                'price_4':number_format(price_4, 2),
