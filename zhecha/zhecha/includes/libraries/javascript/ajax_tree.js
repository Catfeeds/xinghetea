...
$(function()
{
    ...
    $('img[ectype="flex"]').click(function(){
        ...
        if(status == 'open')
        {
            ...
            "<td class='handler bDiv' style='background:none; width:380px; text-align:left;'><a class='btn blue' href='index.php?app=gcategory&amp;act=edit&amp;id="+res[i].cate_id+"'><i class='fa fa-pencil-square-o'></i>"+lang.edit+"</a> <a class='btn red' href='javascript:goConfirm(\""+lang.confirm_delete+"\",\"index.php?app=gcategory&act=drop&id="+res[i].cate_id+"\",true);'><i class='fa fa-trash-o'></i>"+lang.drop+"</a> <a  class='btn green' href='index.php?app=gcategory&amp;act=add&amp;pid="+res[i].cate_id+"'><i class='fa fa-plus'></i>"+lang.add_child+"</a><a href='index.php?app=props&act=distribute&cate_id="+res[i].cate_id+"' class='btn blue'><i class='fa fa-list-ul'></i>"+lang.distribution_props+"</a></td></tr>";
                    }
...
function secajax(ob)
{
    ...
    var src = $(ob).attr('src');
        if(status == 'open')
        {
            ...
            $.get(selfurl + "&act=ajax_cate", {id: id}, function(data){
                if(data)
                {
                    ...
                    for(var i = 0; i < res.length; i++)
                    {
                        ...
                        if(res[i].add_child)
                        {
                            add_child =  " <a  class='btn green' href='index.php?app=gcategory&amp;act=add&amp;pid="+res[i].cate_id+"'><i class='fa fa-plus'></i>"+lang.add_child+"</a>";
                        }
            ...
            "<td class='align_center'><span class='editable' ectype='inline_edit' fieldname='sort_order' fieldid='"+res[i].cate_id+"' datatype='number'>"+res[i].sort_order+"</span></td>"+
            "<td class='align_center'>"+if_show+"</td>"+
			"<td class='handler bDiv' style=' background:none; width:380px; text-align:left;'><a class='btn blue' href='index.php?app=gcategory&amp;act=edit&amp;id="+res[i].cate_id+"'><i class='fa fa-pencil-square-o'></i>"+lang.edit+"</a> <a class='btn red' href='javascript:goConfirm(\""+lang.confirm_delete+"\",\"index.php?app=gcategory&act=drop&id="+res[i].cate_id+"\",true);'><i class='fa fa-trash-o'></i>"+lang.drop+"</a>" + add_child + "<a href='index.php?app=props&act=distribute&cate_id="+res[i].cate_id+"' class='btn blue'><i class='fa fa-list-ul'></i>"+lang.distribution_props+"</a></td></tr>";
                    }
                    pr.after(str);
                    change_background();
                    $('span[ectype="inline_edit"]').unbind('click');
