...
function change_quantity(store_id, rec_id, spec_id, input){
    ...
    $.getJSON(REAL_SITE_URL+'/index.php?app=cart&act=update&spec_id=' + spec_id + '&quantity=' + _v, function(result){
        if(result.done){
	    	...
	    	$('.J_ItemSubtotal-' + rec_id).html(price_format(result.retval.subtotal) + '+' + result.retval.subtotal_exchange + '积分'); 	//增加积分显示 by PwordC	
	    	...
	    	$('.J_C_T_GoodsKinds').html(result.retval.cart.kinds);
			$('.J_C_T_Amount').html(price_format(result.retval.cart.amount)+ '+' + result.retval.amount_exchange + '积分');//增加积分显示 by PwordC	
