...
class BuyappOutlay extends OutlayDepopay
{
	...
	function _update_order_period($trade_info, $extra_info)
	{
		...
		if($item_info = $apprenewal_mod->checkIsRenewal($extra_info['appid'], $trade_info['user_id']))
		{
			...
			$data = array('expired' => $expired);
		 	$result = $apprenewal_mod->edit($item_info['rid'], $data);
		 	//推送所有店铺
		 	$this->edit_apprenewal($item_info['rid'], $data);
		}
		else
		{
			$time = gmtime();
			$expired = strtotime("+{$period} months", $time);
			$data = array('appid' => $extra_info['appid'], 'user_id'=> $trade_info['user_id'], 'add_time' => $time, 'expired' => $expired);
			$result = $apprenewal_mod->add($data);
			//推送到所有店铺 by PwordC
			$this->batch_apprenewal($trade_info['user_id'],$data);
		}
	 ...
	 * by PwordC
	 */
	function batch_apprenewal($id,$data){
	    $store_mod =& m('store');
	    $apprenewal_mod = &m('apprenewal');
	    //获取其余店铺id
	    $store_id = $id;
	    ...
	    foreach ($other_store_ids as $k=>$v){
	        $data['user_id'] = $v['store_id'];
	        $apprenewal_mod->add($data);
	    }
	}
	/*
	 * 续费，推送所有店铺
	 * by PwordC
	 */
	function edit_apprenewal($id,$data){
	    $apprenewal_mod = &m('apprenewal');
	    $apprenewal_info = $apprenewal_mod->get("rid={$id}");
	    $ids = $apprenewal_mod->find(array(
	       'conditions' => 'rid != '.$apprenewal_info['user_id']." and appid='".$apprenewal_info['appid']."'",
            'fields' => 'rid', 
	    ));
	    foreach ($ids as $k=>$v){
	        $apprenewal_mod->edit($v['rid'],$data);
	    }
	}
}
