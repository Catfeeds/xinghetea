...
class DefaultPassportUser extends BasePassportUser
{
     ...
     *    @return    bool
     */
    function check_email($email, $self_id = 0)
    {
		...
		// 是否排除自己（如果是编辑，需要排查自己）
		if($self_id) {
			$conditions .= ' AND user_id != '. $self_id;
		}
		
        $info = $model_member->get($conditions);
        if (!empty($info))
        {
     ...
     *    @return    bool
     */	
	function check_phone($phone_mob, $self_id = 0) 
	{
		$model_member =& m('member');
    ...
    function msg_filter($message)
    {
        ...
        if(strpos($message, '[/url]') !== FALSE)
        {
			if(PHP_VERSION < 5.5) {
            	...
            	$message = preg_replace_callback("/\[url(=((https?|ftp|gopher|news|telnet|rtsp|mms|callto|bctp|ed2k|thunder|synacast){1}:\/\/|www\.)([^\[\"']+?))?\](.+?)\[\/url\]/is", 
"self::msg_filter_url_preg_callback",$message);
			}
        }
        if(strpos($message, '[/img]') !== FALSE)
        {
			if(PHP_VERSION < 5.5) {
				...
				$message = preg_replace_callback("/\[img\]\s*([^\[\<\r\n]+?)\s*\[\/img\]/is", "self::msg_filter_img_preg_callback_step1", $message);
				$message = preg_replace_callback("/\[img=(\d{1,4})[x|\,](\d{1,4})\]\s*([^\[\<\r\n]+?)\s*\[\/img\]/is", "self::msg_filter_img_preg_callback_step2", $message);
			}
        }
        return nl2br(str_replace(array("\t", '   ', '  '), array('&nbsp; &nbsp; &nbsp; &nbsp; ', '&nbsp; &nbsp;', '&nbsp;&nbsp;'), $message));
    }
	
	function msg_filter_url_preg_callback($matches)
	{
		return $this->parseurl($matches[1], $matches[5]);
	}
	function msg_filter_img_preg_callback_step1($matches)
	{
		return $this->bbcodeurl($matches[1], '<img src="%s" border="0" alt="" />');
	}
	function msg_filter_img_preg_callback_step2($matches)
	{
