...
class ECBaseApp extends BaseApp
{
    ...
    function generateQRCode($code, $params = array())
    {
        ...
        if ($code == 'goods_qrcode') {
			$frame = SITE_URL . '/mobile/index.php?app=goods&id=' . $params['goods_id'].'&store_id='.$params['store_id'];//增加store_id
	...
	function gendtbQRCode($code,$params=array())
	{
		...
		if($src_w == 330) //如果为原始生成的330px图片，则重新生成
		{
	...
	function setAutoLoginCookie($user_id, $user_name, $password, $expire = 0)
	{
		...
		if(!$expire)  $expire = time() + 3600 * 24 * 7;
		setcookie('AutoLogin_'.$info_key, json_encode(array('user_id' => $user_id, 'user_name' => $user_name, 'password' =>$password)), $expire,  "/");
	}
	// 检查COOKIE中的密码是否正确
	static function checkAutoLoginCookie($info_key = 'user_info')
	{
		...
		if(isset($_COOKIE['AutoLogin_'.$info_key]) && trim($_COOKIE['AutoLogin_'.$info_key]) <> '')
		{
			$cookie = json_decode(stripslashes($_COOKIE['AutoLogin_'.$info_key]), true);
			if(is_array($cookie)) 
			{
				/* 验证密码是否正确（有可能用户已经改了密码）*/
				$ms = & ms ();
				$user_id = $ms->user->auth ($cookie['user_name'], $cookie['password'] );
...
class BaseVisitor extends Object
{
    ...
    function BaseVisitor()
    {
		...
		elseif($user_id = ECBaseApp::checkAutoLoginCookie($this->_info_key))
		{
        	...
        	$user_info = $mod_user->get(array(
            	...
            	'fields'        => 'user_id, user_name, reg_time, last_login, last_ip, store_id,sys_user_id',
        	));
    ...
    function _get_detail()
    {
        ...
        $member_info = $model_member->findAll(array(
            ...
            'join'          => 'has_store',                 //关联查找看看是否有店铺
            'fields'        => 'email, password, real_name, logins, ugrade, portrait, store_id, state, sgrade , feed_config,sys_user_id',
