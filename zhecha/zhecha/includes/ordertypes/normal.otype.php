...
class NormalOrder extends BaseOrder
{
    ...
    function submit_order($data)
    {
		...
		{	    
		    //特殊业务，默认买一赠一，入库前复制原有商品为赠品，并单独标记入库 by PwordC
		    foreach ($goods_info['orderList'][$store_id]['items'] as $k=>$v){
		        $gift = $v;
		        $gift['price'] = 0;
		        $gift['erpgift'] = 1;
		        $goods_info['orderList'][$store_id]['items'][$k.'gift'] = $gift;
		    }
		    //dump($goods_info['orderList'][$store_id]['items']);die();
	...
	{	
        ...
        foreach ($goods_info['items'] as $key => $value)
        {
            $goods_items[] = array(
                ...
                'erpgift'       =>  isset($value['erpgift'])?$value['erpgift']:0,//增加erpgift字段 by PwordC
            );
        }
