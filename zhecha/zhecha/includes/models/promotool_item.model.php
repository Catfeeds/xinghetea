	...
	function savePromotoolItem($data)
	{
		...
		if(isset($data['config']) && !empty($data['config'])){
            ...
            if($this->_appid == 'exclusive'){
                if($data['config']['discount']) $data['config']['discount'] = abs(number_format($data['config']['discount'], 1));
                if($data['config']['decrease']) $data['config']['decrease'] = abs(number_format($data['config']['decrease'], 2));
