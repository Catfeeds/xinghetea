...
class Delivery_templateModel extends BaseModel
{
	...
	function getTrueCityId($region_id = 0)
	{
		define('CITY_ID_LAYER', 3); // 定义城市所在级数
		$region_mod = &m('region');
		$parents = $region_mod->get_parents($region_id);
		sort($parents);
		
		return array('city_id' => $parents[CITY_ID_LAYER-1], 'porovince_id' => $parents[CITY_ID_LAYER-2]);
	}
	
	function get_type_logist($logist, $destination, $types)
	{
		$logist_fee = array();
		
		// 通过运送目的地destination, 找出city_id（城市id）,获取该城市的上级id，即省id，如果 $city_id=0（等于0是在通过淘宝IP数据库无法返回正常的城市名称的前提下出现，当出现该情况时，取默认运费，即设置：province_id=1
		if($destination)
		{
			/*
			$region_mod = &m('region');
			$region = $region_mod->get(array('conditions'=>'region_id='.$destination,'fields'=>'parent_id'));
			$porovince_id = $region['parent_id'];
			$city_id = $destination;
			*/
			
			$data = $this->getTrueCityId($destination);
			$city_id = $data['city_id'];
			$porovince_id = $data['porovince_id'];
