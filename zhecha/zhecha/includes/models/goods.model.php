...
class GoodsModel extends BaseModel
{
    ...
    var $_relation = array(
		...
		//一个商品对应一个最低抵扣的积分
		'has_goodsintegral' => array(
            'model'         => 'goods_integral',
            'type'          => HAS_ONE,
            'foreign_key'   => 'goods_id',
            'dependent'     => true
        ),
		...
		//商品和促销活动（减价打折）是一对多关系 /  edit by psmb
        'has_limitbuy' => array(
            'model' => 'limitbuy',
            'type'  => HAS_ONE,
            'foreign_key' => 'goods_id',
	...
	//by psmoban.com 获取商品的动态评分
	
	function get_goods_dynamic_evaluation($goods_id)
	{
		$ordergoods_mod=&m('ordergoods');
		$g_evaluation=$ordergoods_mod->find(array(
			'conditions'=>'goods_id='.$goods_id.' AND evaluation_status=1 AND is_valid=1',
			'join'      => 'belongs_to_order',
			'fields'        => 'goods_evaluation',
		));
		$evaluation_count = count($g_evaluation);
		$total = 0;
		
		if(!empty($g_evaluation))
		{
			foreach($g_evaluation as $key=>$val)
			{
				$total = $total+$val['goods_evaluation'];	
			}
		}
		$avg_g_eva=$evaluation_count > 0 ? $total/$evaluation_count : 0;
		return  $avg_g_eva;
	}
    ...
    function get_list($params = array(), $scate_ids = array(), $desc = false, $no_picture = true)
    {
		...
		$goods_pvs_mod = &m('goods_pvs');  // sku psmb 
        ...
        $fields .= "g.goods_id, g.type, g.goods_name, g.cate_id, g.cate_name, g.brand, g.spec_qty, g.spec_name_1, g.spec_name_2, g.if_show, g.closed, g.add_time, g.recommended, g.default_image, " .
                "gs.spec_id, gs.spec_1, gs.spec_2, gs.color_rgb, gs.price,gs.price_1,gs.price_2,gs.price_3,gs.price_4,gs.price_5, gs.stock, " .
                "s.store_name, s.region_id, s.region_name, s.credit_value, s.sgrade, " .//不关联店铺，改为默认为旗舰店。by PwordC
				"gp.pvs, " .  // sku psmb 
                "gst.views, gst.sales, gst.comments";
        $desc && $fields .= ", g.description";
        $tables = "{$this->table} g " .
                "LEFT JOIN {$gs_mod->table} gs ON g.default_spec = gs.spec_id " .
                "LEFT JOIN {$store_mod->table} s ON s.store_id = 2 " .//不关联店铺，改为默认为旗舰店。by PwordC
				"LEFT JOIN {$goods_pvs_mod->table} gp ON g.goods_id=gp.goods_id " .  // sku psmb 
                "LEFT JOIN {$gstat_mod->table} gst ON g.goods_id = gst.goods_id ";

        /* 条件(WHERE) */
        $conditions = $this->_getConditions($conditions, true);
        $conditions .= " AND gs.spec_id IS NOT NULL ";
        if ($scate_ids)
        {
		...
		foreach ($goods_list as $key => $goods) {
			...
			if($result !== FALSE) {
				$goods_list[$key]['price'] = $result['pro_price'];
				$goods_list[$key]['pro_type'] = $result['pro_type'];
			}
		}
    ...
    {
        $goods = $this->get(array(//取消店铺关联，by PwordC
            'conditions' => "g.goods_id = '$id'",
            'join'       => 'has_goodsintegral',
            'fields'     => 'this.*,gi.max_exchange, g.goods_id'
        ));
        if ($goods)
        {
            /* 商品规格 */
    ...
    function get_count_of_store($store_id)
    {
        ...
        if (!isset($data[$store_id]))
        {
            ...
            if($data === false)
            {
                ...
                //共享商品，不用分组查询 by PwordC
                $sql = "SELECT COUNT(*) AS goods_count FROM ecm_goods WHERE if_show = 1 AND closed = 0";
                $data = array();
                //获取所有店铺id by PwordC
                $store_mod =& m('store');
                $ids = $store_mod->getCol("select store_id from ecm_store");
                
                $res = $this->db->query($sql);
                $row = $this->db->fetchRow($res);
                //复制数组 by PwordC
                foreach ($ids as $id){
                    $data[$id] = $row['goods_count'];
    ...
    //业务模型方法挪到普通模型中，业务模型禁用，by PwordC ----------------------------------------------------------------------------
    ...
    /* 过滤掉不是本店的商品id ，取消店铺id限制，by PwordC*/
    function get_filtered_ids($ids)
    {
        $sql = "SELECT goods_id FROM {$this->table} WHERE goods_id " . db_create_in($ids);
    
        return $this->db->getCol($sql);
    }
    
    /* 取得商品数 ，取消店铺id限制，by PwordC*/
    function get_count()
    {
