...
class GcategoryBModel extends GcategoryModel
{
    ...
    function get_children($id, $cached = false)
    {
        ...
        {
            $sql = "SELECT cate_id, cate_name, parent_id FROM {$this->table} WHERE parent_id = '$id' AND store_id = '{$this->_store_id}'";
            $res = $this->getAll($sql);
        }
    ...
    function get_ancestor($id, $cached = false)
    {
        ...
        if ($cached)
        {
            ...
            if ($id > 0 && isset($data[$id]))
            {
                while ($id > 0)
                {
                    $res[] = array('cate_id' => $id, 'cate_name' => $data[$id]['name'], 'parent_id' => $data[$id]['pid']);
                    $id    = $data[$id]['pid'];
                }
            }
        }
        else
        {
            while ($id > 0)
            {
                ...
                if ($row)
                {
                    $res[] = array('cate_id' => $row['cate_id'], 'cate_name' => $row['cate_name'], 'parent_id' => $row['parent_id']);
                    $id    = $row['parent_id'];
                }
				else
				{
					// 删掉分类后， 没有删除该分类下的商品，导致编辑此商品会卡死，所以，当找不到记录的时候，赋值0，避免死循环
					$id = 0;
				}
            }
        }
