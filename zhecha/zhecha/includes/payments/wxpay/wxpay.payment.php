...
class WxpayPayment extends BasePayment
{
	...
	*/
    ...
    function verify_notify($orderInfo, $strict = false)
    {
        ...
        if ($orderInfo['amount'] != round($notify['cash_fee']/100,2))
        {
            /* 支付的金额与实际金额不一致 */
		...
		/* 按通知结果返回相应的结果 */
		if($notify['return_code'] == TRUE && $notify['result_code'] == TRUE)
		{
	...
	function _getNotifySpecificData()
	{
		...
		return array(round($notify['cash_fee']/100,2), $notify['transaction_id'], 'payment_bank' => $notify['bank_type']);
	}	
}
