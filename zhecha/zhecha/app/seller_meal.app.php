...
class Seller_mealApp extends StoreadminbaseApp
{
    ...
    function index()
    {
        ...
        $this->_curlocal(LANG::get('member_center'),    'index.php?app=member',
                         LANG::get('seller_meal'), 	'index.php?app=seller_meal',
                         LANG::get('meal_list'));
    ...
    function add()
    {
        ...
        else
        {
			...
			if($meal_id = $this->_meal_mod->add($data)) {
			    //推送优惠信息到所有店铺 by PwordC
			    $this->batch_meal('add',$meal_id,$data);
				if ($_POST['file_id']) {
	...
	function drop()
	{
		...
		if($this->_meal_mod->drop('user_id='.$this->_store_id.' AND meal_id='.$id)){
			...
			//删除推送信息。
			$this->_meal_mod->drop("parent_id={$id}");
			return;
		}
     ...
     * by PwordC
     */
    function batch_meal($type,$id,$data){
        ...
        if ($type=='add'){
            foreach ($store_ids as $store_id){
                ...
                $new_data['user_id'] = $store_id;
                $new_data['parent_id'] = $id;
                $this->_meal_mod->add($new_data);
            }
        }
        if ($type=='edit'){
            $infos = $this->_meal_mod->find(array(
                'conditions'=>"parent_id={$id}"
            ));
            //dump($infos);die();
            foreach ($infos as $k=>$v){           
                    $new_data = $data;
                    //addLog('selectid', $new_data['selected_ids']);

                    $new_data['user_id'] = $v['user_id'];
                    $this->_meal_mod->edit_data($v['meal_id'],$new_data,$v['meal_id']);
