...
class My_storeApp extends StoreadminbaseApp
{
    ...
    function index()
    {
        ...
        if (!IS_POST)
        {
            ...
            $this->import_resource('jquery.plugins/jquery.validate.js,mlselection.js,pickcolor.js');
	...
	function map()
	{
		if(!IS_POST)
		{
			$store = $this->_store_mod->get(array('conditions'=>'store_id='.$this->_store_id, 'fields'=>'latlng'));
	...
	function slides()
	{
		...
		else 
		{
			...
			$all_slides = array();
			if($pic_slides_arr === false) $pic_slides_arr = array();
			
			
			if(empty($store['pic_slides'])){
				$all_slides = $pic_slides_arr;
			} else {
			...
			$this->_store_mod->edit($this->_store_id,array('pic_slides'=>json_encode($all_slides)));
    ...
    function drop_slides()
    {
		...
		$del = false;
		$store = $this->_store_mod->get(array('conditions'=>'store_id='.$this->visitor->get('manage_store'), 'fields'=>'pic_slides'));
		if($store && !empty($store['pic_slides']))
		{
			$pic_slides_arr = json_decode($store['pic_slides'],true);
			...
			foreach($pic_slides_arr as $key=>$slides){
				if($key == $file_id) {
					unlink($slides['url']);
					unset($pic_slides_arr[$key]);
					$del = true;
				}
			}
			if($del && $this->_store_mod->edit($this->_store_id, array('pic_slides' => json_encode($pic_slides_arr))))
			{
