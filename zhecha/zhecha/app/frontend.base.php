...
class FrontendApp extends ECBaseApp
{
    ...
    function display($tpl)
    {
		...
		$carts_top  = Psmb_init()->get_carts_top(SESS_ID, $this->visitor->get('user_id'));
		...
		$this->assign('header_gcategories', $this->get_header_gcategories());
		...
		$this->assign('icp_number', Conf::get('icp_number'));
		 ...
		 /* 浏览历史 */
        if(APP != 'goods') $this->assign('g_history', $this->_get_goods_history());
	...
	function get_header_gcategories() 
	{
		$cache_server =& cache_server();
        $key = 'header_gcategories';
        ...
        if($data === false)
        {
			...
			$data = Psmb_init()->get_header_gcategories(0, $position, 1);
    ...
    function login()
    {
        ...
        if (!IS_POST)
        {
            ...
            $this->import_resource(array('script' => 'jquery.plugins/jquery.validate.js,jquery.plugins/poshy_tip/jquery.poshytip.min.js','style'  => 'jquery.plugins/poshy_tip/tip-yellowsimple/tip-yellowsimple.css'));
...
// 			$this->assign('alipayconnect_open', $this->_get_enabled_plugins('on_alipay_login', 'alipayconnect') ? 1 : 0);
    ...
    function _do_login($user_id)
    {
		...
		{
			$this->show_warning('your_account_has_locked');
			return false;
		}
...
class MemberbaseApp extends MallbaseApp
{
    ...
    function _curitem($item)
    {
		...
		if(!$this->visitor->get('has_store')){
			unset($member_menu['im_seller'], $member_menu['promotool']);
    ...
    function _get_member_menu()
    {
		...
		{
			$menu['my_ecmall']['submenu']['my_integral'] = array(
				 'text'  => Lang::get('my_integral'),
		...
		/* 应用市场 */
// 		$menu['my_ecmall']['submenu']['appmarket']  = array(
...
//                 ),
        ...
        if ($this->visitor->get('manage_store'))
        {
			...
			{
				//if($msg_setting['msg_pid'] && $msg_setting['msg_key']) {
...
//                 'submenu'   => array(),
//             );
// 			$menu['promotool']['submenu']['distribution_manage'] = array(
//                     'text'  => Lang::get('distribution_manage'),
//                     'url'   => 'index.php?app=my_distribution',
//                     'name'  => 'distribution_manage',
//                     'icon'  => 'ico10',
//             );
// 			$menu['promotool']['submenu']['limitbuy_manage'] = array(
...
// 					'icon'  => 'ico7',
// 			);
// 			$menu['promotool']['submenu']['seller_fullgift'] = array(
// 					'text'  => Lang::get('fullgift'),
// 					'url'   => 'index.php?app=seller_fullgift',
// 					'name'  => 'fullgift',
// 					'icon'  => 'ico2',
// 			);
// 			$menu['promotool']['submenu']['seller_growbuy'] = array(
...
class StorebaseApp extends FrontendApp
{
    ...
    function get_store_data()
    {
        ...
        if ($store === false)
        {
			...
			{
				$banner_info = @getimagesize($store['store_banner']);
				$store['store_banner_height'] = $banner_info[1] ? $banner_info[1] : 119;
			}
			...
			// 店铺二维码
			$store['qrcode'] = $this->generateQRCode('store_qrcode', array('store_id' => $this->_store_id));
	...
	function _get_hot_saleslist()
	{
	   ...
	   $goods_mod =& m('goods');
       $data = $goods_mod->find(array(//取消店铺限制，by PwordC
