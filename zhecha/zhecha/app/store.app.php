...
class StoreApp extends StorebaseApp
{
    function index()
    {
        ...
        /* 取得推荐商品 */
        $this->assign('recommended_goods', $this->_get_recommended_goods($id,20));
        $this->assign('new_groupbuy', $this->_get_new_groupbuy($id));

        /* 取得最新商品 */
        $this->assign('new_goods', $this->_get_new_goods($id,20));
		
		/* 取得热卖商品 */
		$this->assign('hot_sale_goods', $this->_get_hot_sale_goods($id,20));
    ...
    function groupbuy()
    {
        ...
        $groupbuy_list = $groupbuy_mod->find(array(
            'fields'    => 'goods.default_image,goods.price,gb.group_name, gb.group_id, gb.spec_price, gb.end_time, gb.state',
            'join'      => 'belong_goods',
        ...
        foreach ($groupbuy_list as $key => $_g)
        {
			...
			$groupbuy_list[$key]['original_price'] = $_g['price'];
            ...
            else
            {
				...
				$groupbuy_list[$key]['left_time'] = Psmb_init()->lefttime($_g['end_time']);
            }
        }
        /* 当前位置 */
        $this->_curlocal(LANG::get('all_stores'), 'index.php?app=search&amp;act=store',
    ...
    function _get_recommended_goods($id, $num = 12)
    {
        ...
        $goods_list = $goods_mod->find(array(
            ...
            'fields'     => 'goods_name, default_image, price,sales,comments',
            'limit'      => $num,
        ));
        foreach ($goods_list as $key => $goods)
        {
            empty($goods['default_image']) && $goods_list[$key]['default_image'] = Conf::get('default_goods_image');
			$goods_list[$key]['goods_evaluation'] =$goods_mod->get_goods_dynamic_evaluation($goods['goods_id']);
        }
    ...
    function _get_new_groupbuy($id, $num = 12)
    {
        ...
        $groupbuy_list = $model_groupbuy->find(array(
            'fields'    => 'goods.default_image,goods.price, this.group_name, this.group_id, this.spec_price, this.end_time',
            'join'      => 'belong_goods',
