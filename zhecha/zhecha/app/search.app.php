...
class SearchApp extends MallbaseApp
{
    ...
    function index()
    {
        ...
        $goods_list = $goods_mod->get_list(array(
			...
			//'fields'=>'s.praise_rate,s.im_qq,s.im_ww,',//不关联店铺，单独赋值，默认为旗舰店。by PwordC
            'limit'      => $page['limit'],
        ));
		...
		$this->assign('provinces', $this->_get_province_list());
		
        $this->display('search.goods.html');
    }
	
	// 地区筛选优化，读取所有省份列表
	function _get_province_list()
	{
		$region_id = empty($_GET['region_id']) ? 0 : intval($_GET['region_id']);
		
		$region_mod = &m('region');
		$countries = $provinces = array();
		$countries = $region_mod->find(array('conditions'=>'parent_id=0','fields'=>'region_id'));
		
		foreach($countries as $key=>$country) {
			$province = $region_mod->find(array('conditions'=>'parent_id='.$country['region_id'],'fields'=>'region_id,region_name'));
			foreach($province as $k=>$v) {
				if(($v['region_id']==$region_id) || ($region_mod->get_layer($region_id)==3 && $this->getprovince($region_id,'parent_id')==$v['region_id'])) {
		...
		/* 设置选中的省份,城市 */
		$curprovince = $curcity = array();
		if($region_mod->get_layer($region_id)==3) 
		{
			$curcity = $region_mod->get(array('conditions'=>'region_id='.$region_id,'fields'=>'parent_id,region_name'));
			$curprovince = $region_mod->get(array('conditions'=>'region_id='.$curcity['parent_id'],'fields'=>'region_name'));	
		}
		else {
			$curprovince = $region_mod->get(array('conditions'=>'region_id='.$region_id,'fields'=>'region_name'));	
		}
		
		return array('list' => $provinces, 'selected' => array('province' => $curprovince['region_name'], 'city'=> $curcity['region_name']));
	}
	function getprovince($region_id, $fields='')
	{
		$region_mod = &m('region');
		$province = $region_mod->get(array('conditions'=>'region_id='.$region_id,'fields'=>'parent_id,region_id,region_name'));
		if($province) {
			if(empty($fields)) return $province;
			else return $province[$fields];
		}
		return $region_id;
	}
	function getcity()
	{
		...
		// 如果地区是城市级别，则取父级ID(即省)
		if($region_mod->get_layer($region_id)==3) {
			$region = $region_mod->get(array('conditions'=>'region_id='.$region_id,'fields'=>'parent_id'));
			$region_id = $region['parent_id'];
		}
		$cities = $region_mod->find(array('conditions'=>'parent_id='.$region_id,'fields'=>'region_id,region_name'));
		foreach($cities as $key=>$city){
			if($city['region_id']==$_GET['region_id']) $cities[$key]['selected'] = 1;
		}
		
		$this->json_result($cities);
	}
	...
	function _format_goods_list($goods_list)
	{
		...
		foreach ($goods_list as $key => $goods)
        {
			...
			/* 商品所在地*/
			$region_name = str_replace("\t", ' ', $goods['region_name']);
			$region_name_arr = explode(' ', $region_name);
			$goods_list[$key]['address'] = $region_name_arr[count($region_name_arr)-1];
        }
		return $goods_list;
	}
	function _get_ultimate_store()
	{
		...
		return Psmb_init()->get_ultimate_store($conditions, $brand);
	}
	/* 列表页排行，推荐商品 */
    ...
    function store()
    {
        ...
        $stores = $model_store->find(array(
            ...
            'limit'   =>$page['limit'],
			'fields'  =>'store_name,user_name,sgrade,store_logo,address,praise_rate,credit_value,s.im_qq,im_ww,business_scope,region_name,avg_goods_evaluation,avg_service_evaluation,avg_shipped_evaluation ',
        ...
        foreach ($stores as $key => $store)
        {
			$goods_list = $model_goods->find(array(
				'conditions'=>'if_show=1 AND closed=0 ',//取消店铺id限制 by PwordC
				'order'     =>'add_time desc',
				'limit'     =>10,
    ...
    function groupbuy()
    {
        ...
        foreach ($groupbuy_list as $key => $groupbuy)
        {
            ...
            $groupbuy['state'] == GROUP_ON && $groupbuy_list[$key]['lefttime'] = Psmb_init()->lefttime($groupbuy['end_time']);
    ...
    function _recommended_groupbuy($_num)
    {
        ...
        foreach ($data as $gb_id => $gb_info)
        {
            ...
            $data[$gb_id]['lefttime']   = Psmb_init()->lefttime($gb_info['end_time']);
    ...
    {
        /* 组成查询条件 */
        $conditions = " g.if_show = 1 AND g.closed = 0 "; // 上架且没有被禁售，店铺是开启状态,//取出店铺限制条件，默认显示旗舰店 by PwordC
        if (isset($param['keyword']))
        {
