...
class My_goodsApp extends StoreadminbaseApp
{
	...
	function publish()
	{
        $gcategories = $this->_get_gcategory(0);  // 商城分类第一级
		
		$this->assign('gcategories', $gcategories);
		
		$action = intval($_GET['id']) > 0 ? 'edit' : 'add';
		...
		$publish_article = $article_mod->get(array('conditons'=>'article_id=1', 'fileds'=>'content'));
		$this->assign('article', $publish_article);
		$this->display('my_goods.category.html');
	}
	
	/* 取得商城商品分类JSON数据， 同时处理点击和检索 */
	function ajax_gcategory()
	{
		$cate_id = intval($_GET['cate_id']);
		$multilevel = intval($_GET['multilevel']);
		
		$data = array();
		if(!$multilevel) {
			$gcategories = $this->_get_gcategory($cate_id);
			$data[$cate_id] = $gcategories;
		}
		else
		{
			/* 获取他的上级及本身 */
			$gcategory_mod =& bm('gcategory', array('_store_id' => 0));
			$parent = $gcategory_mod->get_ancestor($cate_id);
			
			$parent_ids = array();
			foreach($parent as $value)
			{
				$parent_ids[] = $value['cate_id'];
			}
			
			$i = 0;
			foreach($parent as $key=>$val) {
				$gcategories = $gcategory_mod->find(array('conditions'=>'if_show = 1 AND parent_id='.$val['parent_id'], 'fields'=>'cate_id,cate_name,parent_id'));
				foreach($gcategories as $k => $v) {
					if($gcategory_mod->get_children($v['cate_id'])) {
						$gcategories[$k]['hasChild'] = 1;						
					} else $gcategories[$k]['hasChild'] = 0;
					
					if(in_array($v['cate_id'], $parent_ids)) {
			...
			/* 检索出来的分类的最后一级是否有子分类 */
			if($children = $gcategory_mod->get_children($cate_id)) {
				foreach($children as $child_key => $child_val){				
					if($gcategory_mod->get_children($child_val['cate_id'])) {
	...
	/* 通过关键检索类目 */
	function ajax_search_gcategory()
	{
		$data = array();
		
		$keyword = html_script(trim($_GET['keyword']));
		
		if(!empty($keyword))
		{
			$gcategory_mod =& bm('gcategory', array('_store_id' => 0));
			$gcategories = $gcategory_mod->find(array('conditions'=>"if_show = 1 AND cate_name LIKE '%".$keyword."%' "));
			
			/*  寻找分类的上级，组成完整的分类路径 */
			foreach($gcategories as $key=>$val)
			{
				$parent = $gcategory_mod->get_ancestor($val['cate_id']);
	...
	/*  获取下一级的分类 */
	function _get_gcategory($cate_id = 0)
	{
		$gcategory_mod =& bm('gcategory', array('_store_id' => 0));
        $gcategories = $gcategory_mod->find(array(
			'conditions' => 'if_show=1 AND parent_id='.$cate_id,
			'order' => 'sort_order, cate_id',
			'fields' => 'cate_id,cate_name,parent_id',
		));
		foreach($gcategories as $key=>$val)
		{
			if($gcategory_mod->get_children($val['cate_id'])) {
    ...
    function add()
    {
        //判断是否开启允许添加产品配置，by PwordC
        if ($this->_get_add_goods_setting('status') != 1){
            $this->show_warning('该功能暂不可用...');
        }
        //增加是否为旗舰店判断，by PwordC
        if ($this->visitor->get('manage_store') != 2){
            $this->show_warning('非旗舰店不可用...');
        }
        /* 检测支付方式、配送方式、商品数量等 */
        if (!$this->_addible()) {
            return;
		...
		/* 没有选择分类，跳转至选择分类页面 */
		if(!$cate_id) {
			header('Location:index.php?app=my_goods&act=publish');
			exit;
		}
        ...
        {			
			...
			/* 取得当前商品分类的属性 */
			$this->assign('prop_list', $this->_get_gcategory_props($cate_id,0));
			...
			$this->assign('brand_list', $this->_brand_mod->find(array('conditions'=>'(store_id = ' . $this->_store_id . ' OR store_id=0) AND if_show = 1')));
			 ...
			 /* 赋值运费模板 - psmb */
			 $this->assign('deliverys', $this->_get_delivery_template());
    ...
    function edit()
    {
		...
		//判断会员等级价格设置状态 by PwordC
		if ($this->_get_vip_price_setting('status') == 1){
		    $config = $this->_get_vip_price_setting('config');
		    $config = unserialize($config);
		    $this->assign('price',$config);
		}
		//dump($config);die();
        if (!IS_POST)
        {
			...
			/* 取得当前商品分类的属性 */
			$this->assign('prop_list', $this->_get_gcategory_props($goods['cate_id'], $id));
   ...
   function save_spec($spec)
   {
        ...
        //增加会员价格，by PwordC
        if (!empty($spec['price_1'])){
            foreach ($spec['price_1'] as $key => $val)
            {
                $data[$key]['price_1'] = $this->_filter_price($val);
            }
        }
        if (!empty($spec['price_2'])){
            foreach ($spec['price_2'] as $key => $val)
            {
                $data[$key]['price_2'] = $this->_filter_price($val);
            }     
        }
        if (!empty($spec['price_3'])){
            foreach ($spec['price_3'] as $key => $val)
            {
                $data[$key]['price_3'] = $this->_filter_price($val);
            }
        }
        if (!empty($spec['price_4'])){
            foreach ($spec['price_4'] as $key => $val)
            {
                $data[$key]['price_4'] = $this->_filter_price($val);
            }
        }
        if (!empty($spec['price_5'])){
            foreach ($spec['price_5'] as $key => $val)
            {
                $data[$key]['price_5'] = $this->_filter_price($val);
            }
        }   
    ...
    function import_taobao()
    {
        ...
        else
        {
            ...
            foreach ($records as $record)
            {
                ...
                /* 商品分类 */
                if (isset($_POST['sgcate_id']) && !empty($_POST['sgcate_id']))
                {
...
// 			),
		...
		//判断是否是旗舰店，是否允许操作商品 by PwordC
		if ($this->visitor->get('user_id') == 2){
		    //dump('11');die();
		    $this->assign('allow_handle',1);
		}
		
		//判断是否开启允许添加产品配置，by PwordC
		if ($this->_get_add_goods_setting('status') == 1 && $this->visitor->get('manage_store') == 2){
        ...
        switch ($goods['spec_qty'])
        {
            case 0: // 没有规格
                ...
                if (isset($_POST['price_1'])){
                    $specs[intval($_POST['spec_id'])]['price_1'] = $this->_filter_price($_POST['price_1']);
                }
                if (isset($_POST['price_2'])){
                    $specs[intval($_POST['spec_id'])]['price_2'] = $this->_filter_price($_POST['price_2']);
                }
                if (isset($_POST['price_3'])){
                    $specs[intval($_POST['spec_id'])]['price_3'] = $this->_filter_price($_POST['price_3']);
                }
                if (isset($_POST['price_4'])){
                    $specs[intval($_POST['spec_id'])]['price_4'] = $this->_filter_price($_POST['price_4']);
                }
                if (isset($_POST['price_5'])){
                    $specs[intval($_POST['spec_id'])]['price_5'] = $this->_filter_price($_POST['price_5']);
                }
                break;
            case 1: // 一个规格
                ...
                foreach ($spec_data as $key => $spec_1)
                {
                    ...
                    if ($spec_1)
                    {
                        if (($spec_id = intval($_POST['spec_id'][$key]))) // 已有规格ID的
                        {
                            ...
                            //添加会员多等级价格  by PwordC
                            if (isset($_POST['price_5'])){
                                $specs[$key]['price_5'] = $this->_filter_price($_POST['price_5'][$key]);
                            }
                            if (isset($_POST['price_4'])){
                                $specs[$key]['price_4'] = $this->_filter_price($_POST['price_4'][$key]);
                            }
                            if (isset($_POST['price_3'])){
                                $specs[$key]['price_3'] = $this->_filter_price($_POST['price_3'][$key]);
                            }
                            if (isset($_POST['price_2'])){
                                $specs[$key]['price_2'] = $this->_filter_price($_POST['price_2'][$key]);
                            }
                            if (isset($_POST['price_1'])){
                                $specs[$key]['price_1'] = $this->_filter_price($_POST['price_1'][$key]);
                            }
    ...
    function _check_post_data($data, $id = 0)
    {
		...
		if(($max_exchange > 0 && ($max_exchange >= max($spec))) || ($data['max_exchange'] < 0) || ($max_exchange < 0)){
			$this->show_warning('max_exchange_illege');
			return;
		}
		if($data['exclusive']['decrease']){
		    if($data['exclusive']['decrease'] >= min($spec)){
		        $this->show_warning('exclusive_decrease_illege');
		        return;
		    }
		}
		...
		if(!isset($data['goods']['delivery_template_id']) || intval($data['goods']['delivery_template_id'])<0){
			$this->show_warning('select_delivery_template');
			return;
		}
        ...
        {
            $this->show_warning('fill_spec_name');
            return;
        }
        if (empty($data['specs']))
        {
            $this->show_warning('fill_spec');
            return;
        }
