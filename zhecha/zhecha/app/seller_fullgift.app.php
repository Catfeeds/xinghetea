...
class Seller_fullgiftApp extends StoreadminbaseApp
{
	...
	function index()
	{
		...
		$this->_curlocal(LANG::get('member_center'),    'index.php?app=member',
								LANG::get('fullgift'), 'index.php?app=seller_fullgift',
    ...
    {		
		...
		else
		{
			...
			//dump(unserialize($data['rules']));die();
			if(!($add_id = $this->_fullgift_mod->add($data)) && $this->_fullgift_mod->has_error()) {
				$this->show_warning($this->_fullgift_mod->get_error());
				return;
			}
			//推送到所有店铺 by PwordC
			$this->batch_gift('add',$data,$add_id);
	...
	function edit()
    {
		...
		if(!IS_POST)
		{
			...
			$itemList = array();
			$items = isset($fullgift['rules']['items']) ? $fullgift['rules']['items'] : array();
			foreach($items as $id)
			{
		...
		else
		{
			...
			//修改所有店铺满赠信息 by PwordC
			$this->batch_gift('edit', $data,$id);
	...
	function drop()
    {
        ...
        $ids = explode(',', $id);
        //删除所有店铺满赠信息 by PwordC
        $this->batch_gift('drop', '',$ids);
	...
	function itemadd()
	{
		...
		else
		{
			...
			//推送到所有店铺 by PwordC
// 			if ($goods_id){
// 			    $this->batch_gift('itemadd', $data,$goods_id);
// 			}
	...
	function itemedit()
	{
		...
		else
		{
			...
			if(!$this->_gift_mod->edit($id, $data)) {
				$this->show_warning($this->_gift_mod->get_error());
				return;
			}
			//修改所有推送产品
			//$this->batch_gift('itemedit', $data,$id);
	...
	function itemdrop()
    {
        ...
        //删除铺货信息
        //$this->batch_gift('itemdrop', $data,$ids);
     ...
     * by PwordC
     */
    function batch_gift($type,$data,$id=0){
        $store_mod =& m('store');
        //获取其余店铺id
        ...
        if ($type=='add'){
            foreach ($other_store_ids as $k=>$v){
                $new_data = $data;
                $new_data['store_id'] = $v['store_id'];
                $new_data['parent_id'] = $id;
                $this->_fullgift_mod->add($new_data);
            }
        }
        if ($type=='edit'){
            $info = $this->_fullgift_mod->get("psid={$id}");
            $psids = $this->_fullgift_mod->find(array(
                'conditions' => "appid = '".$info['appid']."' and parent_id = {$id}",
                'fields' => 'psid,store_id',
            ));
            foreach ($psids as $k=>$v){
                $new_data = $data;
                $new_data['store_id'] = $v['store_id'];
                $this->_fullgift_mod->edit($v['psid'],$new_data);
            }
        }
        if ($type=='drop'){
            
            foreach ($id as $v){
                $info = $this->_fullgift_mod->get("psid={$v}");
                
                $psids = $this->_fullgift_mod->find(array(
                    'conditions' => "appid = '".$info['appid']."' and parent_id = '".$v."' and store_id !=".$store_id,
                    'fields' => 'psid',
                ));
                $ids = array();
                foreach ($psids as $k=>$v){
                    array_push($ids, $v['psid']);                  
                }
                
                $bool = $this->_fullgift_mod->drop($ids);
            }
           
        }
        
        //赠品共用，注释以下代码。by PwordC  -------------------------------------------------------------------------
//         if($type=='itemadd'){
//             foreach ($other_store_ids as $v){
//                 $data['store_id'] = $v['store_id'];
//                 $data['parent_id'] = $id;
//                 $this->_gift_mod->add($data);
//             }
//         }
//         if ($type == 'itemedit'){
//             $gift_ids = $this->_gift_mod->find(array(
//                 'conditions' => 'parent_id ='.$id,
//                 'fields' => 'goods_id,store_id'
//             ));
//             foreach ($gift_ids as $k=>$v){
//                 $data['store_id'] = $v['store_id'];
//                 $this->_gift_mod->edit($v['goods_id'],$data);
//             }
//         }
//         if ($type == 'itemdrop'){
//             $batch_ids = array();
//             foreach ($id as $batch_id){
//                 $gift_infos = $this->_gift_mod->find(array(
//                    'conditions' => 'parent_id='.$batch_id 
//                 ));
//                 foreach ($gift_infos as $gift_info){
//                     $this->_gift_mod->drop_data($gift_info['goods_id'],$gift_info['store_id']);
//                     $this->_gift_mod->drop($gift_info['goods_id']);
//                 }
//             }
//         }
//---------------------------------------------------------------------------------------------------------------
