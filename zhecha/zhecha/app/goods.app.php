...
//         dump($data);die();
    ...
    function qa()
    {
        ...
        else
        {
            ...
            {
                header("Location: index.php?app=goods&act=qa&id={$id}&store_id={$store_id}#module\n");
                exit;
            }
            else
            {
    ...
    function _get_common_info($id,$store_id)//增加store_id，by PwordC
    {
        ...
        if ($data === false)
        {
			...
			// 商品二维码
			$goods['qrcode'] = $this->generateQRCode('goods_qrcode', array('goods_id' => $goods['goods_id'],'store_id'=>$store_id));//增加store_id
